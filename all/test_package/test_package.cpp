...
bool compare_r(const string_tree &left,
               const string_tree::const_iterator &left_i,
               const string_tree &right,
               const string_tree::const_iterator &right_i) {
  bool result = true;
  // compare this node, then recursively compare the children
  if (*left_i != *right_i) {
    std::cerr << "left = \"" << *left_i << "\" is different from right = \""
              << *right_i << "\"" << std::endl;
    result = false;
  }
  if (left.children(left_i) != right.children(right_i)) {
    std::cerr << "left = \"" << *left_i
              << "\" has different number of children from right = \""
              << *right_i << "\"" << std::endl;
    result = false;
  } else {
    for (unsigned i = 0; i < left.children(left_i); i++)
      result &= compare_r(left, left.child(left_i, i), right,
                          right.child(right_i, i));
  }
  return result;
}
...
bool compare(const string_tree &left, const string_tree &right) {
  ...
  if (left.empty() && right.empty())
    return true;
  if (left.empty()) {
    std::cerr << "different size - left is empty right = " << right.size()
              << std::endl;
    return false;
  }
  if (right.empty()) {
    std::cerr << "different size - right is empty left = " << left.size()
              << std::endl;
    return false;
  }
...
void dump_string_tree_iterator(stlplus::dump_context &context,
                               const string_tree::iterator &data) {
  stlplus::dump_ntree_iterator(context, data);
}
void restore_string_tree_iterator(stlplus::restore_context &context,
                                  string_tree::iterator &data) {
  stlplus::restore_ntree_iterator(context, data);
}

void print_string_tree_iterator(std::ostream &device,
                                const string_tree::iterator &data) {
  if (data.null())
    device << "<null>";
  else if (data.end())
...
void print_string_tree_const_iterator(std::ostream &device,
                                      const string_tree::const_iterator &data) {
  print_string_tree_iterator(device, data.deconstify());
}

void print_string_tree_iterator_vector(
    std::ostream &device, const string_tree::iterator_vector &data) {
  device << data.size() << ": ";
  stlplus::print_vector(device, data, print_string_tree_iterator, ", ");
}

void print_string_tree_const_iterator_vector(
    std::ostream &device, const string_tree::const_iterator_vector &data) {
  device << data.size() << ": ";
  stlplus::print_vector(device, data, print_string_tree_const_iterator, ", ");
}

std::ostream &operator<<(std::ostream &device,
                         const string_tree::iterator &data) {
...
std::ostream &operator<<(std::ostream &device,
                         const string_tree::prefix_iterator &data) {
  print_string_tree_iterator(device, data.simplify());
  return device;
}

std::ostream &operator<<(std::ostream &device,
                         const string_tree::postfix_iterator &data) {
  print_string_tree_iterator(device, data.simplify());
  return device;
}

std::ostream &operator<<(std::ostream &device,
                         const string_tree::iterator_vector &data) {
  print_string_tree_iterator_vector(device, data);
  return device;
}

std::ostream &operator<<(std::ostream &device,
                         const string_tree::const_iterator_vector &data) {
...
void dump_tree_map(stlplus::dump_context &context, const tree_map &data) {
  stlplus::dump_map(context, data, stlplus::dump_string,
                    dump_string_tree_iterator);
}

void restore_tree_map(stlplus::restore_context &context, tree_map &data) {
  stlplus::restore_map(context, data, stlplus::restore_string,
                       restore_string_tree_iterator);
}

std::ostream &operator<<(std::ostream &device, const tree_map &mappings) {
  device << "map: ";
  stlplus::print_map(device, mappings, stlplus::print_string,
                     print_string_tree_iterator, "=", ",");
  device << std::endl;
  return device;
}
...
// usage: build a tree and then call add_mappings to built the tree map from the
// tree
...
public:
  ...
  void add_mappings(void) {
    ...
    m_map.clear();
    for (string_tree::prefix_iterator i = m_tree.prefix_begin();
         i != m_tree.prefix_end(); i++) {
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    std::cerr << "empty breadth-first traversal = "
              << data.m_tree.breadth_first_traversal() << std::endl;
    ...
    string_tree::iterator right_right =
        data.m_tree.insert(right, "right_right");
    ...
    std::cerr << "breadth-first traversal = "
              << data.m_tree.breadth_first_traversal() << std::endl;
    ...
    string_tree::prefix_iterator found = std::find(
        data.m_tree.prefix_begin(), data.m_tree.prefix_end(), "left_left");
    if (found.valid()) {
    ...
    string_tree::postfix_iterator not_found = std::find(
        data.m_tree.postfix_begin(), data.m_tree.postfix_end(), "not_found");
    if (!not_found.valid()) {
    ...
    // get breadth-first traversal
    std::cerr << "breadth-first traversal = "
              << ((const string_tree)data.m_tree).breadth_first_traversal()
              << std::endl;
    ...
    try {
      ...
      std::cerr << "ERROR: failed to throw exception for iterator" << illegal
                << std::endl;
      result = false;
  ...
  if (left_offset != 0) {
    std::cerr << "ERROR: left offset is wrong, should be 0, is " << left_offset
              << std::endl;
    result = false;
  }
  ...
  if (right_offset != 2) {
    std::cerr << "ERROR: right offset is wrong, should be 1, is "
  ...
  if (not_found_offset != (unsigned)-1) {
    std::cerr << "ERROR: not found offset is wrong, should be -1, is "
              << not_found_offset << std::endl;
    result = false;
  }
  ...
  if (reordered_left_offset != 1) {
    std::cerr << "ERROR: reordered left offset is wrong, should be 1, is "
              << reordered_left_offset << std::endl;
    result = false;
  }
  ...
  std::cerr << "reordered middle offset: " << reordered_middle_offset
            << std::endl;
  if (reordered_middle_offset != 2) {
    std::cerr << "ERROR: reordered middle offset is wrong, should be 2, is "
              << reordered_middle_offset << std::endl;
    result = false;
  }
  ...
  try {
    ...
    std::cerr << "ERROR: failed to throw out of range exception in reorder"
              << std::endl;
    result = false;
  } catch (std::exception &except) {
    std::cerr << "success: caught standard exception " << except.what()
              << std::endl;
  } catch (...) {
  ...
  std::cerr << "reordered right offset: " << reordered_right_offset
            << std::endl;
  if (reordered_right_offset != 2) {
    std::cerr << "ERROR: reordered right offset is wrong, should be 1, is "
              << reordered_right_offset << std::endl;
    result = false;
  }
